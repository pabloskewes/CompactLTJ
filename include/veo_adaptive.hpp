...
template <
    class ltj_iterator_t =
        ltj_iterator_v2<cltj::compact_ltj, uint8_t, uint64_t>,
...
public:
  ...
  veo_adaptive(
    ...
    /*for(const auto & v : m_var_info){
        cout << "var=" << (uint64_t) v.name << " weight=" << v.weight <<
    endl;
    }*/
    // cout << "Done. " << endl;
  }
  ...
  //! Copy constructor
  veo_adaptive(const veo_adaptive &o) { copy(o); }

  //! Move constructor
  veo_adaptive(veo_adaptive &&o) { *this = move(o); }
  ...
  inline var_type next() {
    if (m_index < m_var_info.size()) { // No lonely
      ...
      m_not_bound.erase(min_iter); // Remove the variable from a list of
                                   // unbound variables
      ++m_index;
  ...
  inline void up() {
    if (m_index - 1 < m_var_info.size()) { // No lonely
      for (const update_type &update :
           m_versions.top()) { // Restart the weights
  ...
  inline void done() {
    ...
    if (m_index < m_var_info.size()) { // No lonely
      ...
      m_not_bound.push_back(pos); // Restart the list of unbound
                                  // variables
  ...
  inline size_type size() { return m_var_info.size() + m_lonely.size(); }

  inline size_type nolonely_size() { return m_var_info.size(); }
};
}; // namespace veo
} // namespace ltj
