...
template <class Trie> class cltj_index_metatrie {
public:
...
private:
  ...
  trie_type create_full_trie(vector<spo_triple> &D, uint8_t order) {
    ...
    /*            cout << "*** Trie para el orden " << (uint64_t)order << "
       ***" << endl; for (uint64_t i = 0; i < bv.size(); i++) { cout <<
       bv[i];
                }
                cout << endl;
...
typedef cltj::cltj_index_metatrie<cltj::compact_trie_v3> compact_ltj_metatrie;
