...
class ltj_iterator { // TODO: if CLTJ is eventually integrated with the ring to
                     // form a Compact Index Schemes project then this class
                     // has to be renamed to CLTJ_iterator, for instance.
...
private:
  ...
  // std::vector<std::string> m_orders = {"0 1 2", "0 2 1", "1 2 0", "1 0 2",
  // "2 0 1", "2 1 0"};
  //                                      SPO      SOP      POS      PSO OSP
  //                                      OPS
  ...
  //                                                 0)};
  // std::array<std::vector<size_type>, 2> m_pos_v =
...
public:
  ...
  */
  ...
  // Reverses the intervals and variable weights. Also resets the current
  // value.
  ...
  bool
  exists(state_type state, size_type c) { // Return the minimum in the range
  ...
  leap(var_type var, size_type c = -1ULL) { // Return the minimum in the range
    ...
    // if(c == -1) return key(); //TODO: improve this, we can get the key
    // from beg
    size_type value, pos;
    if (c == -1ULL) {
      value = trie->seq[beg];
    ...
    if (m_nfixed == 0) {
      ...
      // std::cout << "It0: " << m_status[m_nfixed+1].it[0] << " It1: " <<
      // m_status[m_nfixed+1].it[1] << " Trie: " << m_trie_i << " Status:
      //" << m_status_i << std::endl;
    } else {
  ...
  inline size_type subtree_size_fixed1(state_type state
  ) const { // TODO: review this
