...
class ltj_iterator_v2 { // TODO: if CLTJ is eventually integrated with the ring
                        // to form a Compact Index Schemes project then this
                        // class has to be renamed to CLTJ_iterator, for
                        // instance.
...
private:
  ...
  size_type trie_switch() {
    ...
    const auto *trie =
        m_ptr_index->get_trie(trie_aux); // switches to the corresponding trie
  ...
  */
  ...
  void down(state_type state) {
    ...
    if (m_nfixed == 1) {
      ...
      m_status[m_nfixed].it[1] = trie_aux->nodeselect(
          m_status[m_nfixed].beg - 1
      ); // -1 as there is no root in this trie
    } else if (m_nfixed == 2) {
      if (!m_status_i) {
        const auto *trie = m_ptr_index->get_trie(m_trie_i);
        auto pos = m_status[m_nfixed].beg;
