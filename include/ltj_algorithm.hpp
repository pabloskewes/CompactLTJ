...
template <
    class iterator_t = ltj_iterator_v2<cltj::compact_ltj, uint8_t, uint64_t>,
...
private:
  ...
  inline void
  add_var_to_iterator(const var_type var, ltj_iter_type *ptr_iterator) {
...
public:
  ...
  ltj_algorithm(
    ...
    m_veo = veo_type(
        m_ptr_triple_patterns, &m_iterators, &m_var_to_iterators, m_ptr_ring
    );
  }
    ...
    if (j == m_veo.size()) {
      ...
      if (itrs.size() == 1 && itrs[0]->in_last_level()) { // Lonely variables
        ...
        for (const auto &c : results) {
          ...
          tuple[j] = {x_j, c};
          // 2. Going down in the trie by setting x_j = c (\mu(t_i) in
          // paper)
          itrs[0]->down(x_j, c);
          m_veo.down();
        ...
        while (c != 0) { // If empty c=0
          // 1. Adding result to tuple
          tuple[j] = {x_j, c};
          // 2. Going down in the tries by setting x_j = c (\mu(t_i)
          // in paper)
  ...
  value_type seek(const var_type x_j, value_type c = -1) {
    ...
    while (true) {
      ...
      // std::cout << "Leap of " << (::uint64_t) x_j << " in iterator: "
      // << i << std::endl;
      if (c == -1) {
        c_i = itrs[i]->leap(x_j);
      } else {
  ...
  void
  print_results(vector<tuple_type> &res, unordered_map<uint8_t, string> &ht) {
    cout << "Results: " << endl;
    uint64_t i = 1;
