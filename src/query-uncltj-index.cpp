...
void query(
  ...
  if (result) {
    ...
    for (string &query_string : dummy_queries) {
      ...
      typedef ltj::ltj_iterator_v2<index_scheme_type, uint8_t, uint64_t>
          iterator_type;
#if ADAPTIVE
      ...
      typedef ltj::ltj_algorithm<
          ...
          algorithm_type;
#endif
      // typedef std::vector<typename algorithm_type::tuple_type>
      // results_type; typedef algorithm_type::results_type results_type;
      ...
      // cout << "RESULTS QUERY " << count << ": " << number_of_results <<
      // endl;
      count += 1;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    std::cout << "Usage: " << argv[0] << " <index> <queries> <limit> <type>"
              << std::endl;
    return 0;
  }
  ...
  if (type == "normal") {
    query<cltj::uncompact_ltj, ltj::util::trait_distinct>(
        index, queries, limit
    );
  } else if (type == "star") {
